        ...
        for i in range(0, height):
            ...
            if ls_parent_multiple_position:
                ...
                for parent_multiple in ls_parent_multiple_position:
                    ...
                    # the childrent positions of the former one should be removed before process following trees
...
def new_entry():
    ls_trees = LexicaBinaryTree.load_trees()
    for tree in ls_trees:
        str_tree = LexicaBinaryTree(tree)
        draw_trees(ParentedTree.fromstring(tree))
        binary_tree = str_tree.get_flat_binary_tree()
        draw_trees(ParentedTree.fromstring(binary_tree))
        print(binary_tree)
